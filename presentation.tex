...
\begin{frame}{Stencil computations}
  % show LHS FEM assembly pseudocode
  % show RHS generalised stencil algorithm
\end{frame}
...
\begin{frame}{Stencil library wishlist}
  ...
  \unchecked Map composition \\
  \unchecked Nesting loops (e.g. PCPATCH) \\
...
\begin{frame}
  \begin{paragraph}{Claim:}
    \pyop3's new data layout abstraction enables all of these. 
  \end{paragraph}
\end{frame}
...
\begin{frame}{Sparse matrices}
  % same as ragged but change the inner indices so they miss some out
  % also don't provide code
\end{frame}
...
\begin{frame}{Something really crazy}
  % multi-part, nested ragged and renumbered
\end{frame}

\section{\pyop3 interface}

\begin{frame}{...}
  % - use DMPlex terminology
  % users create a mesh object and data structures that live on it
\end{frame}

\begin{frame}{DMPlex}
  % - how it works (Hasse diagram)
  % - covering relations
  % closure = "cell and incident edges and vertices"
\end{frame}
