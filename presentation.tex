...
\begin{frame}{What came before: \pyop2}
  \begin{itemize}
    ...
    \item Used everywhere in Firedrake for things like residual assembly and interpolation
  \end{itemize}
\end{frame}

\begin{frame}{\pyop2 data model}
  \begin{itemize}
    \item Data is stored by \py{Dats}\footnote{Sparse matrix support is not discussed}
...
\begin{frame}{Introducing \pyop3}
  ...
  \begin{itemize}
    ...
    \item \textbf{Has a new data layout model}
  \end{itemize}
\end{frame}
...
\begin{frame}{\pyop3 wishlist}
  ...
  \unchecked Exploit mesh partial structure (e.g. extruded)\footnotemark \\
  \unchecked Prescribe DoF ordering (e.g. extruded columns)\footnotemark[\value{footnote}] \\

  (And more\dots)

  \footnotetext{Achievable in \pyop2 but difficult to use}
\end{frame}
...
\begin{frame}{Things I missed}
  \begin{itemize}
    \item The interface for launching computations (inc. map and loop composition)
    \item Tight integration with PETSc (esp. DMPlex)
    \item Support for sparse matrices
    \item MPI parallelism
    \item Could streamline PCPATCH and multigrid code (via composition)
